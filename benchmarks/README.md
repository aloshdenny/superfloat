...
benchmarks/
  ...
  modal/
    ...
    modal_vjepa_train.py V-JEPA 2 PTQ and QAT benchmarks (UCF101)
  train_vjepa_probe.py V-JEPA 2 frozen-backbone probe (PTQ)
...
scripts here; the V-JEPA weight-distribution rows in the full results document come from
`modal/modal_vjepa.py` (inference only), and the V-JEPA probe accuracies from
`train_vjepa_probe.py`.

Figures are in [figures/](figures/), all generated by `make_figures.py` in one
style: solid log-scale validation loss on the left axis, dashed accuracy or
mAP50-95 on the right, and a vertical marker at the peak-validation epoch.

Runs that never learned are excluded from the trajectory panels and listed with
their reason in `EXCLUDED` at the top of `make_figures.py` — currently only
`dota_random_sf4`, which sat at 0.0000 for 263 epochs. They are not hidden:
they are the subject of `failure_modes.png`, where a flat line at zero is the
point rather than noise on a shared axis. The collapsed SF8 lr-4e-3 runs are
...
`train_vjepa_probe.py` quantizes a **frozen** V-JEPA 2 backbone and trains only
an fp32 attentive probe on its features — V-JEPA's own evaluation protocol, and
the right test for a format the paper scopes as deployment-time: it measures how
much of a *trained* representation survives quantization, rather than whether
the backbone can be retrained through the grid.
