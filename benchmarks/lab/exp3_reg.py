"""Experiment 3: does the coarse-grid advantage survive more data?

Tier D found the ordering inverted under scale absorption: at 11m, SF2 (+0.108)
beat SF3 (+0.152) beat SF4 (+0.187), consistently across three seeds. The
hypothesis was regularisation -- an 11m model on 106M tokens is
over-parameterised for its budget, and a coarser grid constrains it harder.

That makes a sharp prediction: the inversion should weaken and then flip as
tokens-per-parameter grows. This sweeps a 5m model across 5x/10x/20x/40x token
budgets at SF2/3/4/6 plus an FP32 control per budget.

If coarse-is-better dies by 40x, regularisation is confirmed. If it survives,
the explanation is wrong and something more interesting is happening.
"""
...
D_MODEL, N_LAYER, N_HEAD = 256, 6, 4
...
def main():
    ...
    tag = f"exp3_tpp{a.tpp}_" + ("fp32" if a.bits==0 else f"sf{a.bits}") + f"_s{a.seed}"
    ...
    rec={"exp":"exp3","bits":a.bits,"tpp":a.tpp,"seed":a.seed,"n_nonembed":n_ne,
